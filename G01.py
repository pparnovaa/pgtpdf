...
def create_download_link(val, filename):
    ...
    return f'<a href="data:application/octet-stream;base64,{b64.decode()}" download="{filename}.pdf">Скачать PDF</a>'
...
def print_pdf():
    if (calculated):
        ...
        p_workspace_width = pdf.w-pdf.l_margin-pdf.r_margin
        # Подключаем шрифты  
        pdf.add_font('DejaVu', '', 'DejaVuSans.ttf', uni=True)
        pdf.add_font('DejaVu', 'B', 'DejaVuSans-Bold.ttf', uni=True)
        # Заголовок
        pdf.set_font('DejaVu', 'B', 14)
        pdf.cell(p_workspace_width, 10, 'Курсовая работа по курсу "Паровые и газовые турбины"', ln=1)
        pdf.cell(p_workspace_width, 10, 'Выполнила: Парнова Екатерина', ln=1)
        # КПД
        pdf.image('kpd1.png', w=p_workspace_width)
        # h-s диаграмма
        pdf.image('h_s_1.png', w=p_workspace_width)
        pdf.ln(10)
        # Зависимость КПД от давления в конденсаторе(таблица)
        # Выводим заголовок
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width*0.4, 10, 'Давление в конденсаторе', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'КПД', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'G_0', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'G_k', 1, 1, align='C')
        # Заполняем таблицу данными
        pdf.set_font('DejaVu', '', 12)
        for i in range(0, len(itog['Давление в конденсаторе'])):
            pdf.cell(p_workspace_width*0.4, 10, f"{itog['Давление в конденсаторе'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.2, 10, f"{itog['КПД'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.2, 10, f"{itog['G_0'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.2, 10, f"{itog['G_k'][i]:.4f}", 1, 1, align='C')

        # Максимальный КДП (Таблица)
        # Выводим заголовок
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width, 10, 'Максимальный КДП:', 0, 1)
        pdf.cell(p_workspace_width*0.4, 10, 'Давление в конденсаторе', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'КПД', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'G_0', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, 'G_k', 1, 1, align='C')
        # Заполняем таблицу данными
        pdf.set_font('DejaVu', '', 12)
        pdf.cell(p_workspace_width*0.4, 10, f"{itog['Давление в конденсаторе'][0]:.4f}", 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, f"{itog['КПД'][0]:.4f}", 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, f"{itog['G_0'][0]:.4f}", 1, 0, align='C')
        pdf.cell(p_workspace_width*0.2, 10, f"{itog['G_k'][0]:.4f}", 1, 1, align='C')

        # Задание 2
        pdf.set_font('DejaVu', 'B', 14)
        pdf.cell(p_workspace_width, 10, 'Задание 2', ln=1)
        # Зависимость ucf от eta
        pdf.image('ucf_eta.png', w=p_workspace_width)
        pdf.ln(10)
        # Зависимость параметров от H_0 (Таблица)
        # Выводим заголовок
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width, 10, 'Зависимость параметров от H_0:', 0, 1)
        pdf.cell(p_workspace_width*0.25, 10, 'h_0', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.25, 10, 'eta_ol', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.25, 10, 'alpha', 1, 0, align='C')
        pdf.cell(p_workspace_width*0.25, 10, 'U_cf', 1, 1, align='C')
        # Заполняем таблицу данными
        pdf.set_font('DejaVu', '', 12)
        for i in range(0, len(f['h_0'])):
            pdf.cell(p_workspace_width*0.25, 10, f"{f['h_0'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.25, 10, f"{f['eta_ol'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.25, 10, f"{f['alpha'][i]:.4f}", 1, 0, align='C')
            pdf.cell(p_workspace_width*0.25, 10, f"{f['U_cf'][i]:.4f}", 1, 1, align='C')

        # h-s диаграмма
        pdf.image('h_s_2.png', w=p_workspace_width)
        # Треугольник скоростей
        pdf.image('speed_triangle.png', w=p_workspace_width)
        pdf.ln(10)
        # Внутренний относительный КДП ступени (текст)
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width, 10, 'Внутренний относительный КПД ступени', 0, 1)
        pdf.set_font('DejaVu', '', 12)
        pdf.cell(p_workspace_width, 10, f'eta_oi = {eta_oi}', 0, 1)
        pdf.ln(10)
        # Внутренняя мощность ступени (текст)
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width, 10, 'Внутренняя мощность ступени', 0, 1)
        pdf.set_font('DejaVu', '', 12)
        pdf.cell(p_workspace_width, 10, f'N_i = {N_i}', 0, 1)
        pdf.ln(10)

        # Задание3
        pdf.set_font('DejaVu', 'B', 14)
        pdf.cell(p_workspace_width, 10, 'Задание 3', ln=1)
        # Исходные данные
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(p_workspace_width, 10, 'Дано:', 0, 1)
        pdf.set_font('DejaVu', '', 12)
        pdf.cell(p_workspace_width, 10, f'P0 = {P0:.4f} МПа', 0, 1)
        pdf.cell(p_workspace_width, 10, f'h0 = {h0:.4f} кДж/кг', 0, 1)
        pdf.cell(p_workspace_width, 10, f'dрс = {drs:.4f} м', 0, 1)
        pdf.cell(p_workspace_width, 10, f'Z = {Z} шт', 0, 1)
        pdf.cell(p_workspace_width, 10, f'G0 = {G0:.4f} кг/с', 0, 1)
        pdf.cell(p_workspace_width, 10, f'n = {n} Гц', 0, 1)
        pdf.cell(p_workspace_width, 10, f'eta_oi = {eta_oi:.4f}', 0, 1)
        pdf.cell(p_workspace_width, 10, f'Pz = {Pz:.4f} МПа', 0, 1)
        
        # Таблица
        pdf.add_page(orientation='landscape')
        l_workspace_width = pdf.w - pdf.l_margin - pdf.r_margin
        # Выводим заголовок
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(l_workspace_width*0.125, 10, '№ ступени', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.11, 10, 'di, м', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.12, 10, 'li, м', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.125, 10, 'θi', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.125, 10, 'ρi', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.125, 10, 'Xi', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.125, 10, 'Hi, кДж/кг', 1, 0, align='C')
        pdf.cell(l_workspace_width*0.15, 10, 'Hi + Δ, кДж/кг', 1, 1, align='C')

        # Заполняем таблицу данными
        pdf.set_font('DejaVu', '', 12)
        for i in range(0, len(table['№ ступени'])):
            pdf.cell(l_workspace_width*0.125, 10, f"{table['№ ступени'][i]}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.11, 10, f"{table['di, м'][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.12, 10, f"{table['li, м'][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.125, 10, f"{table['θi '][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.125, 10, f"{table['ρi '][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.125, 10, f"{table['Xi '][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.125, 10, f"{table['Hi, кДж/кг'][i]:.4f}", 1, 0, align='C')
            pdf.cell(l_workspace_width*0.15, 10, f"{table['Hi + Δ, кДж/кг'][i]:.4f}", 1, 1, align='C')

        # 7 рисунков распределения
        pdf.add_page(orientation='portrait')
        pdf.image('pic1.png', w=p_workspace_width)
        pdf.image('pic2.png', w=p_workspace_width)
        pdf.image('pic3.png', w=p_workspace_width)
        pdf.image('pic4.png', w=p_workspace_width)
        pdf.image('pic5.png', w=p_workspace_width)
        pdf.image('pic6.png', w=p_workspace_width)
        pdf.image('pic7.png', w=p_workspace_width)

        # pdf.output('test.pdf', 'F')
        html = create_download_link(pdf.output(dest="S").encode("latin-1"), "ParnovaG_0")
        st.markdown(html, unsafe_allow_html=True)


st.button('Напечатать PDF', on_click=print_pdf)
...
plt.ylabel("")
plt.grid()
plt.savefig("speed_triangle.png")
st.pyplot(cw)
...
st.pyplot(fig)
# Помечаем, что закончили все рассчеты, и сохранили все картинки
calculated = True
